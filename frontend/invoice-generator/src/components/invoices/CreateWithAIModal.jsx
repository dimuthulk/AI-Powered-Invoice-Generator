...
const CreateWithAIModal = ({ isOpen, onClose }) => {
  ...
  const handleGenerate = async () => {
    if (!text.trim()) {
      toast.error("Please enter some details to generate an invoice.");
      return;
    }
        ...
        <div className="bg-white rounded-lg shadow-xl max-w-lg w-full p-6 relative text-left transform transition-all">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-slate-900 flex items-center">
              <Sparkles className="w-5 h-5 mr-2 text-blue-600" /> Create Invoice
              with AI
            </h3>
            <button
              onClick={onClose}
          ...
          <div className="space-y-4">
            <p className="text-sm text-slate-600">
              Paste any text that contains invoice details (like client
              name,items, quantities,and prices) and the AI will attempt to
            ...
            <TextareaField
              ...
              placeholder="E.g. 'Invoice for Client XYZ: 3 hours of design work at $150/hr and 1 logo for $800...'"
              rows={8}
            />
          </div>
