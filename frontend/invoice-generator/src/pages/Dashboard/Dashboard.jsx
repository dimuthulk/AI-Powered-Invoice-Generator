  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        // invoices coming from backend use `total` (see backend/models/Invoice.js)
        // older payloads or other APIs might use `totalAmount`; coerce both to numbers
        const totalPaid = invoices
          .filter((inv) => inv.status === "Paid")
          .reduce(
            (acc, inv) => acc + Number(inv.total ?? inv.totalAmount ?? 0),
            0
          );
    ...
    <div className="space-y-8">
      ...
      <div className="w-full bg-white border-slate-200 rounded-lg shadow-sm shadow-gray-100 overflow-hidden">
        ...
        {recentInvoices.length > 0 ? (
          <div className="w-[90vw] md:w-auto overflow-auto shadow-lg">
            <table className="w-full min-w-[600px] divide-slate-200">
