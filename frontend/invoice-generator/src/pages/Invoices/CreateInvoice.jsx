...
function CreateInvoice({ existingInvoice, onSave }) {
  ...
  useEffect(() => {
    ...
    if (existingInvoice) {
      setFormData((prev) => ({
        ...
        billFrom: {
          ...defaultForm.billFrom,
          ...(existingInvoice.billFrom || {}),
        },
        billTo: {
          ...defaultForm.billTo,
      ...
      const generateInvoiceNumber = async () => {
        ...
        try {
          ...
          // find max sequence for current year (format: INV-YYYY-###)
          let maxSeq = 0;
          invoices.forEach((inv) => {
            const m = /^INV-(\d{4})-(\d+)$/.exec(inv.invoiceNumber);
          ...
          let newInvoiceNumber = `INV-${year}-${nextSeq
            .toString()
            .padStart(3, "0")}`;
          let attempts = 0;
          // ensure uniqueness against fetched invoices
          while (existingNumbers.has(newInvoiceNumber) && attempts < 1000) {
            nextSeq++;
            newInvoiceNumber = `INV-${year}-${nextSeq
              .toString()
              .padStart(3, "0")}`;
            attempts++;
          }
          // fallback to year + timestamp+random if still not unique (very unlikely)
          if (existingNumbers.has(newInvoiceNumber)) {
            newInvoiceNumber = `INV-${year}-${Date.now()
              .toString()
  ...
  const handleSubmit = async (e) => {
    ...
    if (onSave) {
      ...
      try {
        await axiosInstance.post(API_PATHS.INVOICES.CREATE, finalFormData);
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm shadow-gray-100 border border-slate-200">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <InputField
            ...
            placeholders={isGeneratingNumber ? "Generating..." : ""}
            disabled
          />
          <InputField
            label="Invoice Date"
